...
// New creates a new web API client with basic auth credentials.
...
// New creates a new web API client with a bearer token for authentication.
// It takes the following parameters:
// - host: http host (github.com).
// - basePath: any base path for the API client ("/v1", "/v3").
// - scheme: http scheme ("http", "https").
...
func (a *Client) GetStyling(params *GetStylingParams, opts ...ClientOption) (*GetStylingOK, error) {
	...
	op := &runtime.ClientOperation{
		...
		Schemes:            []string{"http"},
		Params:             params,
		Reader:             &GetStylingReader{formats: a.formats},
		Context:            params.Context,
		Client:             params.HTTPClient,
	}
	for _, opt := range opts {
	...
	success, ok := result.(*GetStylingOK)
	if ok {
		return success, nil
	}
	// unexpected success response
	// safeguard: normally, absent a default response, unknown success responses return an error above: so this is a codegen issue
	msg := fmt.Sprintf("unexpected success response for getStyling: API contract not enforced by server. Client expected to get an error, but got: %T", result)
	panic(msg)
}
