// Code generated by go-swagger; DO NOT EDIT.

package users

// This file was generated by the swagger tool.
// Editing this file might prove futile when you re-run the swagger generate command

import (
	"context"
	"net/http"
	"time"

	"github.com/go-openapi/errors"
	"github.com/go-openapi/runtime"
	cr "github.com/go-openapi/runtime/client"
	"github.com/go-openapi/strfmt"

	"github.com/cloudentity/acp-client-go/clients/identitysystem/models"
)

// NewSystemUpdateVerifiableAddressParams creates a new SystemUpdateVerifiableAddressParams object,
// with the default timeout for this client.
//
// Default values are not hydrated, since defaults are normally applied by the API server side.
//
// To enforce default values in parameter, use SetDefaults or WithDefaults.
func NewSystemUpdateVerifiableAddressParams() *SystemUpdateVerifiableAddressParams {
	return &SystemUpdateVerifiableAddressParams{
		timeout: cr.DefaultTimeout,
	}
}

// NewSystemUpdateVerifiableAddressParamsWithTimeout creates a new SystemUpdateVerifiableAddressParams object
// with the ability to set a timeout on a request.
func NewSystemUpdateVerifiableAddressParamsWithTimeout(timeout time.Duration) *SystemUpdateVerifiableAddressParams {
	return &SystemUpdateVerifiableAddressParams{
		timeout: timeout,
	}
}

// NewSystemUpdateVerifiableAddressParamsWithContext creates a new SystemUpdateVerifiableAddressParams object
// with the ability to set a context for a request.
func NewSystemUpdateVerifiableAddressParamsWithContext(ctx context.Context) *SystemUpdateVerifiableAddressParams {
	return &SystemUpdateVerifiableAddressParams{
		Context: ctx,
	}
}

// NewSystemUpdateVerifiableAddressParamsWithHTTPClient creates a new SystemUpdateVerifiableAddressParams object
// with the ability to set a custom HTTPClient for a request.
func NewSystemUpdateVerifiableAddressParamsWithHTTPClient(client *http.Client) *SystemUpdateVerifiableAddressParams {
	return &SystemUpdateVerifiableAddressParams{
		HTTPClient: client,
	}
}

/*
SystemUpdateVerifiableAddressParams contains all the parameters to send to the API endpoint

	for the system update verifiable address operation.

	Typically these are written to a http.Request.
*/
type SystemUpdateVerifiableAddressParams struct {

	// UpdateAddress.
	UpdateAddress *models.UpdateUserVerifiableAddress

	/* IfMatch.

	   A server will only return requested resources if the resource matches one of the listed ETag value

	   Format: etag
	*/
	IfMatch *string

	// IPID.
	IPID string

	// UserID.
	UserID string

	/* XCorrelationID.

	   Unique identifier included in audit events for request tracking
	*/
	XCorrelationID *string

	/* XIdempotencyKey.

	   Key used to safely retry failed requests without duplicate processing
	*/
	XIdempotencyKey *string

	timeout    time.Duration
	Context    context.Context
	HTTPClient *http.Client
}

// WithDefaults hydrates default values in the system update verifiable address params (not the query body).
//
// All values with no default are reset to their zero value.
func (o *SystemUpdateVerifiableAddressParams) WithDefaults() *SystemUpdateVerifiableAddressParams {
	o.SetDefaults()
	return o
}

// SetDefaults hydrates default values in the system update verifiable address params (not the query body).
//
// All values with no default are reset to their zero value.
func (o *SystemUpdateVerifiableAddressParams) SetDefaults() {
	// no default values defined for this parameter
}

// WithTimeout adds the timeout to the system update verifiable address params
func (o *SystemUpdateVerifiableAddressParams) WithTimeout(timeout time.Duration) *SystemUpdateVerifiableAddressParams {
	o.SetTimeout(timeout)
	return o
}

// SetTimeout adds the timeout to the system update verifiable address params
func (o *SystemUpdateVerifiableAddressParams) SetTimeout(timeout time.Duration) {
	o.timeout = timeout
}

// WithContext adds the context to the system update verifiable address params
func (o *SystemUpdateVerifiableAddressParams) WithContext(ctx context.Context) *SystemUpdateVerifiableAddressParams {
	o.SetContext(ctx)
	return o
}

// SetContext adds the context to the system update verifiable address params
func (o *SystemUpdateVerifiableAddressParams) SetContext(ctx context.Context) {
	o.Context = ctx
}

// WithHTTPClient adds the HTTPClient to the system update verifiable address params
func (o *SystemUpdateVerifiableAddressParams) WithHTTPClient(client *http.Client) *SystemUpdateVerifiableAddressParams {
	o.SetHTTPClient(client)
	return o
}

// SetHTTPClient adds the HTTPClient to the system update verifiable address params
func (o *SystemUpdateVerifiableAddressParams) SetHTTPClient(client *http.Client) {
	o.HTTPClient = client
}

// WithUpdateAddress adds the updateAddress to the system update verifiable address params
func (o *SystemUpdateVerifiableAddressParams) WithUpdateAddress(updateAddress *models.UpdateUserVerifiableAddress) *SystemUpdateVerifiableAddressParams {
	o.SetUpdateAddress(updateAddress)
	return o
}

// SetUpdateAddress adds the updateAddress to the system update verifiable address params
func (o *SystemUpdateVerifiableAddressParams) SetUpdateAddress(updateAddress *models.UpdateUserVerifiableAddress) {
	o.UpdateAddress = updateAddress
}

// WithIfMatch adds the ifMatch to the system update verifiable address params
func (o *SystemUpdateVerifiableAddressParams) WithIfMatch(ifMatch *string) *SystemUpdateVerifiableAddressParams {
	o.SetIfMatch(ifMatch)
	return o
}

// SetIfMatch adds the ifMatch to the system update verifiable address params
func (o *SystemUpdateVerifiableAddressParams) SetIfMatch(ifMatch *string) {
	o.IfMatch = ifMatch
}

// WithIPID adds the iPID to the system update verifiable address params
func (o *SystemUpdateVerifiableAddressParams) WithIPID(iPID string) *SystemUpdateVerifiableAddressParams {
	o.SetIPID(iPID)
	return o
}

// SetIPID adds the ipId to the system update verifiable address params
func (o *SystemUpdateVerifiableAddressParams) SetIPID(iPID string) {
	o.IPID = iPID
}

// WithUserID adds the userID to the system update verifiable address params
func (o *SystemUpdateVerifiableAddressParams) WithUserID(userID string) *SystemUpdateVerifiableAddressParams {
	o.SetUserID(userID)
	return o
}

// SetUserID adds the userId to the system update verifiable address params
func (o *SystemUpdateVerifiableAddressParams) SetUserID(userID string) {
	o.UserID = userID
}

// WithXCorrelationID adds the xCorrelationID to the system update verifiable address params
func (o *SystemUpdateVerifiableAddressParams) WithXCorrelationID(xCorrelationID *string) *SystemUpdateVerifiableAddressParams {
	o.SetXCorrelationID(xCorrelationID)
	return o
}

// SetXCorrelationID adds the xCorrelationId to the system update verifiable address params
func (o *SystemUpdateVerifiableAddressParams) SetXCorrelationID(xCorrelationID *string) {
	o.XCorrelationID = xCorrelationID
}

// WithXIdempotencyKey adds the xIdempotencyKey to the system update verifiable address params
func (o *SystemUpdateVerifiableAddressParams) WithXIdempotencyKey(xIdempotencyKey *string) *SystemUpdateVerifiableAddressParams {
	o.SetXIdempotencyKey(xIdempotencyKey)
	return o
}

// SetXIdempotencyKey adds the xIdempotencyKey to the system update verifiable address params
func (o *SystemUpdateVerifiableAddressParams) SetXIdempotencyKey(xIdempotencyKey *string) {
	o.XIdempotencyKey = xIdempotencyKey
}

// WriteToRequest writes these params to a swagger request
func (o *SystemUpdateVerifiableAddressParams) WriteToRequest(r runtime.ClientRequest, reg strfmt.Registry) error {

	if err := r.SetTimeout(o.timeout); err != nil {
		return err
	}
	var res []error
	if o.UpdateAddress != nil {
		if err := r.SetBodyParam(o.UpdateAddress); err != nil {
			return err
		}
	}

	if o.IfMatch != nil {

		// header param if-match
		if err := r.SetHeaderParam("if-match", *o.IfMatch); err != nil {
			return err
		}
	}

	// path param ipID
	if err := r.SetPathParam("ipID", o.IPID); err != nil {
		return err
	}

	// path param userID
	if err := r.SetPathParam("userID", o.UserID); err != nil {
		return err
	}

	if o.XCorrelationID != nil {

		// header param x-correlation-id
		if err := r.SetHeaderParam("x-correlation-id", *o.XCorrelationID); err != nil {
			return err
		}
	}

	if o.XIdempotencyKey != nil {

		// header param x-idempotency-key
		if err := r.SetHeaderParam("x-idempotency-key", *o.XIdempotencyKey); err != nil {
			return err
		}
	}

	if len(res) > 0 {
		return errors.CompositeValidationError(res...)
	}
	return nil
}
